 ...
 *
 */
public class aaa implements GLEventListener {
	...
	float vertices[] =
		{ 0.8f , 0.2f , .0f , 1.0f , 0.2f , 0.2f , .0f , 1.0f , 0.5f , 0.8f , .0f , 1.0f };
	...
	void setupBuffers(GL3 gl) {
		...
		// vertex shader
		this.newFloatVertexAndColorBuffers(gl, this.triangleVAO, this.vertices, this.colorArray, this.vertexLoc, this.colorLoc);
		this.newFloatVertexAndColorBuffers(gl, this.axisVAO, this.verticesAxis, this.colorAxis, this.vertexLoc, this.colorLoc);
	}
	...
	protected void renderScene(GL3 gl) {
		...
		setCamera(0.5f, 0.5f, 2, 0.5f, 0.5f, -1, this.viewMatrix);
	...
	int newShaderFromCurrentClass(GL3 gl, String fileName, ShaderType type) {
		...
		gl.glShaderSource(id, 1, new String[]
			{ shaderSource }, null);
		// compile the shader
